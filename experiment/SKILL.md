...
1. Understand the project: read config if it exists (`resskills.yaml` or `config.yaml`),
   otherwise look at the codebase to figure out what to run, what metric to track, and
   what file to modify. If ambiguous, ask the user.
2. Read the train file and any prepare/eval files for full context.
3. If `experiments/results.tsv` doesn't exist, create it with a header row.
   The columns should match the project's actual metrics. Example:
   ```
   commit	metric_value	memory_gb	status	description
   ```
4. Run baseline: execute the training command, record result as "baseline" with status "keep".
5. Commit: `git add experiments/results.tsv && git commit -m "experiment: baseline [metric=VALUE]"`
...
4. Update `findings.md` ONLY with evidence-backed results (e.g., "X improved metric
   by Y%, p=Z across N runs"). Log interpretive narrative to `research-log.md` instead.
